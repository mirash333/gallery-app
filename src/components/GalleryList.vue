<template>
    <div v-if="items.length" class="gallery-wrapper">
        ...
        <button
            v-if="search.input ? search.meta.page < search.meta.total_pages : items.length && $route.name !== 'CollectionCard'"
            @click="loadMoreItems"
    ...
    <div class="mb-10" v-else>
        <p class="text-center">{{
                search.input ? (errors['searchPhoto'] ? errors['searchPhoto'] : 'No photo for the given search') : 'No photos'
            }}</p>
    </div>
</template>
