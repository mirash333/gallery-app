<template>
    <header class="header bg-gray-50 shadow mb-10">
        <div class="header__container flex container mx-auto justify-between ">
            <router-link to="/" class="header__logo flex duration-200 flex-grow items-center hover:text-green-500">
                ...
                <p class="font-bold">Gallery App</p>
            </router-link>
            <nav class="header__links flex flex-grow items-center justify-end">
                <template v-if="!loading">
                    <template v-for="collection in collections">
                        <div class="ml-3 inline-flex rounded-md shadow">
                            <router-link :to="`/photos/${collection.id}`" :key="collection.id"
                ...
                <template v-else>
                    <template v-for="(collection,index) in collections">
                        <div class="ml-3 inline-flex rounded-md shadow" :key="index">
                            <div class="inline-flex transition duration-100 text-sm items-center justify-center px-3 py-2 border border-transparent font-medium rounded-md text-gray-700 bg-white hover:bg-green-50">
