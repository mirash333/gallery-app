<template>
    <figure class="bg-gray-100 rounded-xl p-8 user shadow-md">
        <img class="w-32 h-32 rounded-full mx-auto" :src="userProfile.profile_image.large" alt="" width="384" height="512">
        <div class="pt-6 space-y-4">
            ...
            <figcaption>
                <div v-if="userProfile.name" class="text-green-500">
                    <p class="text-xl mb-4">Full name: {{userProfile.name}}</p>
                </div>
...
export default {
    ...
    created() {
        if (!this.$route.params.user) {
            this.loading = true
            this.getUser({username: this.$route.params.username}).then(() => this.loading = false)
        }
    }
}
</script>
